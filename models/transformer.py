...
class Encoder(nn.Module):
    ...
    def forward(self, protein):
        ...
        # print("encoder 中的protein",protein.size())    torch.Size([62, 700, 64])
        conv_input = self.fc(protein)
        # print("encoder conv_input", conv_input.size())  # ([62, 700, 64])
        # conv_input=[batch_size, protein_len, hid_dim]

        # permute for convolutional layer
        conv_input = conv_input.permute(0, 2, 1)
        # print("encoder conv_input", conv_input.size())   ([62, 64, 700])
...
class DecoderLayer(nn.Module):
    ...
    def forward(self, trg, src, trg_mask=None, src_mask=None):
        ...
        # print("trg.size(): ", trg.size())      # torch.Size([62, 44, 64])  [batch_size, compound_len, atom_dim]
        # print("src.size(): ", src.size())      #  torch.Size([62, 700, 64])  [batch_size, protein_len, hid_dim]
        trg = self.ln(trg + self.do(self.sa(trg, trg, trg, trg_mask)))    # 化合物  化合物   化合物
        # print("trg1.size():",trg.size())   # torch.Size([62, 44, 64])      torch.Size([62, 46, 64])
        trg = self.ln(trg + self.do(self.ea(trg, src, src, src_mask)))    # 化合物   蛋白质  蛋白质
        # print("trg2.size():", trg.size())  #  torch.Size([62, 44, 64])     torch.Size([62, 46, 64])
        trg = self.ln(trg + self.do(self.pf(trg)))
        # print("trg3.size():", trg.size())  # torch.Size([62, 44, 64])     torch.Size([62, 46, 64])
        return trg
...
class Decoder(nn.Module):
    ...
    def forward(self, trg, src, trg_mask=None, src_mask=None, line=None):
        ...
        # print("norm1.size():",norm.size())  #  torch.Size([62, 43])
        ...
        # print("norm2.size():", norm.size())     # torch.Size([62, 43])
        ...
        #         v = v * norm[i, j]
        #         sum[i, ] += v
        # print("norm.unsqueeze(1).size()：",norm.unsqueeze(1).size())   #  torch.Size([62, 1, 43])
        sum = torch.bmm(norm.unsqueeze(1), trg).squeeze(1)
        # print("sum.size():", sum.size())    #  torch.Size([62, 64])
