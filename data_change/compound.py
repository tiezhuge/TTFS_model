...
def adjacent_matrix(mol):
    adjacency = Chem.GetAdjacencyMatrix(mol)   # 生成分子的邻接矩阵(Adjacent martrix)
    # print("adjacency.shape[0]:",adjacency.shape[0])  # 39  28
    return np.array(adjacency) + np.eye(adjacency.shape[0])   # np.eye(?)生成对角阵，主对角线上元素为1，其余位置均为0
...
if __name__ == "__main__":
   atom_feat,adj_matrix,_= get_mol_features('COc1c(Cl)cc2c([nH]c3cnccc32)c1NC(=O)c1cccnc1C', 34)
   print("a")
